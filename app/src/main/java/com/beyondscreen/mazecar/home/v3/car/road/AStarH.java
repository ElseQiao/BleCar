...
public class AStarH {
	...
	//----------------------------------------------以下为配合游戏使用的方法-------------------------
	private static AStarH aStarH;
	private static AStarH getInstance(){
		if(aStarH==null){
			aStarH=new AStarH();
		}
		return aStarH;
	...
	public static ArrayList<Integer> getRoad(int fromX,int fromY,int toX,int toY){
		Node startNode = new Node(fromX-1, fromY-1);
		Node endNode = new Node(toX-1, toY-1);
	...
	private static Integer exchange(int x, int y) {
		return Position.getIndex(x+1, y+1);//加1是为了对应底图坐标
	}
