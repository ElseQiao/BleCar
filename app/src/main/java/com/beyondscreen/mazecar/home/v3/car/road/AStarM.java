...
public class AStarM {
	...
	//----------------------------------------------以下为配合游戏使用的方法-------------------------
	private static AStarM aStarH;
	private static AStarM getInstance(){
		if(aStarH==null){
			aStarH=new AStarM();
		}
		return aStarH;
	...
	public static ArrayList<Integer> getRoad(int fromX,int fromY,int toX,int toY){
		Node startNode = new Node(fromX-1, fromY-4);
		Node endNode = new Node(toX-1, toY-4);
	...
	private static Integer exchange(int x, int y) {
		return Position.getIndex(x+1, y+4);//加1是为了对应底图坐标
	}
